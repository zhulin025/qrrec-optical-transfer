...
import { unpackTransferFile } from "../shared/file-envelope";
import { fnv1a, parseFrame } from "../shared/protocol";
import { prepareZXingModule, readBarcodes } from "zxing-wasm/reader";
import { zxingWasmDataUrl } from "./zxing-wasm-inline.generated";
  ...
  workerInitPromise = new Promise<boolean>((resolve) => {
    ...
    const startWorker = (slot: number) => {
      ...
      worker.onmessage = (event: MessageEvent) => {
        const { id, bytes, ready, error } = event.data as {
          id: number;
...
function captureFrame() {
  ...
  if (__XHS_MAIN_THREAD__) {
    ...
    void readBarcodes(normalizedImage, {
      ...
      .then((results) => {
        const decoded = results.find((entry) => entry.isValid && entry.bytes.length > 0);
        if (decoded) onDecoded(decoded.bytes);
      })
      .catch(() => undefined)
      .finally(() => {
...
function finish(payload: Uint8Array, hashOk: boolean, seconds: number, totalLen: number) {
  ...
  const transfer = unpackTransferFile(payload);
  ...
  fileMeta.textContent = `${fileName} · ${Math.round(fileBytes.length / 1024)} KB · ${blobType}`;
