...
| **V5.2 全屏显示版** | 验证更大光码对定位率和吞吐量的影响 | 保持 V5.1 编码与原生解码链路不变；开始传输后自动请求浏览器全屏，让彩色矩阵占满可用视口 | 对照测试显示面积、拍摄距离、未定位帧与有效解码 FPS | [全屏发送端](https://qrrec.liuwa.xyz/v5.2/send/) · 接收端沿用 QRREC V5.1 App | 约 146.5 KB/s（7.5 KB × 20 FPS） | **180 KB/s**（B 模式真机测试） |
...
> **速率口径：**理论速率是默认参数下、尚未扣除喷泉冗余和识别丢帧的文件有效载荷上限，统一按 1 KB = 1024 B 换算。实测速率来自当前项目的历史真机测试；V3 彩色矩阵的两次记录均未完成整文件传输，V5/V5.1/V5.2 是 App 内按成功解码帧统计的载荷速率，可能重复统计同一个发送画面中的喷泉数据，因此可能高于理论唯一载荷速率，也不等同于“文件大小 ÷ 完成耗时”的端到端净速率。
...
V5.2 沿用 V5.1 的 20 FPS libcimbar 码流和原生 iOS 接收端，只改变发送画面的空间利用率。点击开始后浏览器会尝试进入系统全屏，隐藏设置区并把彩色矩阵铺满可用视口；结束传输或退出全屏后恢复设置界面。当前真机测试中，普通 B 模式可用，App 显示接收速率达到 180 KB/s；Bm 矩形模式未能正常接收，暂不推荐。说明见 [`v5.2/README.md`](v5.2/README.md)。
